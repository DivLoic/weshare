<?php
	...
	function user_content($id_profil)
	{
		...
		$user_data_query=$bdd->prepare('SELECT id, pseudo, score, url_profile_image,DATE_FORMAT(date_inscription, \'%d/%m/%Y\') AS date, code_postal  FROM utilisateur  WHERE utilisateur.id= ? ');
	...
	function item($id_profil)
	{
		...
		$user_item_query=$bdd->prepare('SELECT annonce.url_image, annonce.vu, annonce.titre, DATE_FORMAT(annonce.date, \'%d/%m/%Y %H:%i\') AS date, rubrique.nom AS rub FROM annonce LEFT JOIN rubrique ON annonce.id_rubrique= rubrique.id WHERE (id_utilisateur=? AND visible=0) ORDER BY annonce.date DESC');
